...
const retryAccepted = args["retry-accepted"] === "true";
const extraSourcePages = [...new Set(String(args["source-pages"] || "").split(",").map(value => value.trim()).filter(Boolean))];
const explicitSourceImage = args["source-image"];
const explicitSourcePage = args["source-image-page"];
if (retryAccepted && !onlyWorks.size) throw new Error("--retry-accepted=true requires --only-work or --only-works");
if ((explicitSourceImage || explicitSourcePage) && (!onlyWork || !explicitSourceImage || !explicitSourcePage)) throw new Error("--source-image and --source-image-page require exactly one --only-work");
...
const retryWorks = previousWorks.filter(work => (!onlyWorks.size || onlyWorks.has(work.workId)) && (retryStatuses.has(work.status) || work.imagePolicy === "museum_hero_placeholder" || (!work.selected && work.objectImageResolved !== true) || (retryAccepted && work.selected)));
...
async function enumeratePage(sourcePageUrl) {
  ...
  try {
    ...
    const raw = await page.evaluate(() => {
      ...
      const nearby = element => (element.closest("figure,.image,[role=figure],.standard__grid__item,[class*='grid__item'],article,section,main")?.textContent || element.parentElement?.textContent || "").trim().slice(0, 800);
      ...
      document.querySelectorAll("img#file, .fullImageLink img, #file img").forEach(element => { const url = largestSrc(element) || element.currentSrc || element.src || element.getAttribute("data-src"); add("direct_image", url, element, {elementKind: "file_page_main_image", explicitRecordRelation: true}); });
...
const rankForWork = (page, identity) => {
  ...
  const candidates = [...page.candidates].map(candidate => {
    ...
    const identityScore = candidate.genericPenalty && !candidate.explicitRecordRelation ? -1000 : (exactOfficialOg ? 2000 : 0) + (candidate.explicitRecordRelation ? 1500 : 0) + (title && text.includes(title) ? 1000 : 0) + (creator && text.includes(creator) ? 300 : 0) + (candidate.selector ? 20 : 0) - (candidate.sourceType === "network_image" ? 10 : 0);
...
for (const item of enumerated) {
  ...
  try {
    ...
    if (chosen.selector && /(?:image returned (?:403|429)|asset is not an allowed image MIME)/i.test(error.message)) {
      try {
        const page = await browser.newPage({viewport: {width: 1440, height: 1000}, deviceScaleFactor: 1});
        try {
          await page.goto(pageInfo.finalUrl || item.sourcePageUrl, {waitUntil: "domcontentloaded", timeout: 45000});
