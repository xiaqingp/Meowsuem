...
if (pool.candidates.length !== 40) throw new Error(`expected 40 candidates, found ${pool.candidates.length}`);
...
for (let index = 0; index < 4; index += 1) {
  ...
  const packet = {
    ...
    museum: pool.museum,
    poolDecision: pool.poolDecision,
    candidates,
    sources: pool.sources.filter(source => sourceIds.has(source.id))
  };
  ...
  const packetRelative = relative(packetPath);
  const header = {
    runId: `m28-6-vienna-fresh-research-batch-${String(index + 1).padStart(2, "0")}-2026-07-23`,
    ...
    works: candidates.map(item => ({
      ...
      workIdentity: {
        artistOrCulture: item.makerOrCulture,
        title: item.nameZh,
        titleEn: item.nameEn,
...
console.log("scaffolded four fresh-research batches of ten candidates");
