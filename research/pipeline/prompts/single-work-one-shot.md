...
```json
{
  ...
  "upstreamConflicts": [
    {
      "field": "creator",
      "lockedValue": "locked metadata value",
      "observedValue": "reliable source value",
      "sourceIds": ["S1"],
      "severity": "warning",
      "explanation": "why the difference does or does not change identity, selection, rating, or publication"
    }
  ]
}
```
...
每项都必须包含 `field`、`lockedValue`、`observedValue`、`severity` 与
